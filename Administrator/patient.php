...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <?php
      if(isset($_POST['delete-patient'])) {
        ...
        // Update the patient record in the database
        $sql = "UPDATE patient SET p_status = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        ...
        if(mysqli_stmt_affected_rows($stmt) > 0) {
          echo '<meta http-equiv="refresh" content="2">';
          echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Patient Deleted successfully</div>';
        } else {
          echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">Patient Delete failed. Please try again.</div>';
        }
                ...
                <div class="col-12">
                    ...
                    <div class="card-header">
                      <div class="clearfix">
                        ...
                        href = "report.php?action=patients" 
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                          <?php
                              ...
                              $stmt = mysqli_prepare($conn, "SELECT patient.id, fname, lname, email, phone, bday, address, county, gender, blood_id, blood_type.b_name FROM patient LEFT OUTER JOIN blood_type ON blood_type.id = patient.blood_id WHERE patient.p_status != 2");
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form name="edit-patient" id="edit-patient" method="post" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>">
          <input type="hidden" id="epatid" name="epatid">
            ...
            <div class="row">
              ...
              <!-- /.col -->
              <div class="col-4">
                <button type="submit" name="edit-patient" class="btn btn-success btn-block">Update</button>
              </div>
              <!-- /.col -->
            </div>
          </form>
  ...
  <div class="modal fade" id="modal-delete">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Confirm Deleting Patient</h4>
          <button type="button" class="close" style="outline:none;" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
        Deleting...
        <h2 class="text-center"><span id="delpatname" style="text-transform: uppercase;"></span></h2>
        </div>
        <form method="post" name="delete-patient" role="delete-patient" action="<?php echo $_SERVER['PHP_SELF']; ?>">
          <input type="hidden" name="delpatId" id="delpatId">
          <div class="modal-footer">
              <!-- <div class="row"> -->
                  <button type="submit" name="delete-patient" class="btn btn-danger btn-block">Delete Patient</button>
          </div>
        </form>
      </div>
      <!-- /.modal-content -->
...
<!-- ./wrapper -->
 ...
 <script>
  ...
  button.addEventListener('click', function() {
    var pat_id = button.getAttribute('data-id');
    ...
    $.ajax({
            ...
            data: {
                id: pat_id,
                action: "returnPatient"
            },
            dataType: "json",
            success: function(response) {
                // Process the response here
                // console.log(response);
                $('#epatid').val(response.id);
  ...
  button.addEventListener('click', function() {
    ...
    $.ajax({
            ...
            success: function(response) {
                // Process the response here
                $('#delpatId').val(response.id);
                $('#delpatname').html(response.fullName);
            }
        });
  });
});
