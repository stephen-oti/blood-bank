...
<head>
  ...
  <title>OBBS | Patient Registration</title>
...
<div class="register-box">
  <div class="register-logo">
    <a href="#"><b style="color: #ff5b56">OBBS | </b>Patient Reg</a>
  </div>
  <?php
    if(isset($_POST['submit'])){
      ...
      $sql = "INSERT INTO patient(fname,lname,email,phone,bday,address,county,pword,gender) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
      ...
      if(!empty($fname) && !empty($lname)&& !empty($mail)&& !empty($phone)&& !empty($dob)&& !empty($address)&& !empty($county)&& !empty($psword)){
        // Execute the statement
          ...
          if(mysqli_stmt_error($stmt)) {
              ...
              echo '<div class="alert bg-danger">Error while updating the database</div>';
          } else {
            $p_id = mysqli_insert_id($conn);
            $user_type = 'patient';

            // Insert a new record into the questionnaire table with default values
            $stmnt = mysqli_prepare($conn, "INSERT INTO questionnaire (user_type, p_id, q1, q2, q3, q4, q5, q6, q7, q8, q9, q10) VALUES (?, ?, 'no', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 'no')");
            mysqli_stmt_bind_param($stmnt, "si", $user_type, $p_id);
  ...
  <div class="card">
    <div class="card-body register-card-body">
      <p class="login-box-msg">Register as a Patient</p>
      <!-- This is where Response happens -->
      <form method="post" name="patient-register" id="patient-register" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>">
